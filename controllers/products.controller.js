        ...
        try {
              ...
              console.log(req.body)
               ...
               if (validation.isValidate()) {
                      ...
                      const { insertId } = await productsModel.updateOne(NOTE,COMMENTAIRE,ID_NOTE)
                      const note = (await productsModel.findByIdNote(insertId))[0]
                      res.status(200).json({
                             success: true,
...
const getNoteUser = async (req, res) => {
        try {
                ...
                const noteListe = await productsModel.findNoteUser(req.userId)
        ...
        try {
                ...
                const notes = {
                        produit_note: {
                                NOTE: note.NOTE,
                                COMENTAIRE: note.COMMENTAIRE,
                                DATE_INSERTION: note.DATE_INSERTION,
...
const DeleteNote= async (req, res) => {
        try {
                const {ID_NOTE} = req.params
                const note = await query("DELETE FROM ecommerce_produit_notes WHERE ecommerce_produit_notes.ID_NOTE=?",[ID_NOTE])
                res.status(RESPONSE_CODES.OK).json({
                        statusCode: RESPONSE_CODES.OK,
                        httpStatus: RESPONSE_STATUS.OK,
                        message: "Suppression reussi",
