...
const ecommerce_commandes_model = require('../../models/ecommerce/ecommerce_commandes.model')
...
const create = async (req, res) => {
          try {
                    ...
                    if (true) {
                              ...
                              const { insertId: ID_DETAILS_LIVRAISON } = await createDetailLivraison(req.userId, shipping_info.NOM, shipping_info.PRENOM, shipping_info.ADRESSE, shipping_info.TELEPHONE, shipping_info.AVENUE, shipping_info.ID_COUNTRY)
                              const livreursQuery = `
                              ...
                              await Promise.all(grouped.map(async (commande) => {
                                        ...
                                        const { insertId } = await ecommerce_commandes_model.createCommandes(
                                                  PAYEMENT_ID,
...
/**
 * Permet de récuperer toutes les commandes
 * @param {express.Request} req
 * @param {express.Response} res 
 */
const getPartenaireCommandes = async (req, res) => {
          try {
                    ...
                    var commandesIds = []
                    const commandes = await ecommerce_commandes_model.getAllCommandes(ID_PARTENAIRE_SERVICE, ID_USER)
                    
                    commandes.forEach(commande => commandesIds.push(commande.ID_COMMANDE))
                    var details = 0
                    if (commandesIds.length > 0) {
                              details = await ecommerce_commandes_model.getManyCommandesDetails(commandesIds)
                    }
...
const updateStatus = async (req, res) => {
          try {
                    ...
                    await ecommerce_commandes_model.saveStatus(ID_COMMANDE, req.userId, ID_STATUT)
