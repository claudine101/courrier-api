...
const findproduct = async (id,category, subCategory, limit = 10, offset = 0) => {
    try {
        ...
        var binds = []
        var sqlQuery = "SELECT ep.ID_PRODUIT,ep.NOM,ep.ID_CATEGORIE_PRODUIT,ep.ID_PRODUIT_SOUS_CATEGORIE,ep.IMAGE_1,ep.IMAGE_2,ep.IMAGE_3,ep.ID_PARTENAIRE_SERVICE,ps.NOM_ORGANISATION,u.NOM AS NOM_USER ,u.PRENOM,"
        sqlQuery += " sp.PRIX,eps.QUANTITE_TOTAL,eps.QUANTITE_VENDUS,eps.QUANTITE_RESTANTE,ps.ID_TYPE_PARTENAIRE"
        ...
        sqlQuery += " partenaire_service ps ON ps.ID_PARTENAIRE_SERVICE= ep.ID_PARTENAIRE_SERVICE "
        ...
        sqlQuery += " LEFT JOIN partenaires p ON p.ID_PARTENAIRE=ps.ID_PARTENAIRE LEFT JOIN users u ON u.ID_USER=p.ID_USER "
        
                  sqlQuery += " LEFT JOIN ecommerce_wishlist_produit wi ON "
                  sqlQuery += " wi.ID_PRODUIT=ep.ID_PRODUIT "
...
const findproducts = async (category, subCategory, limit = 10, offset = 0) => {
    ...
    try {
        var binds = []
        var sqlQuery = "SELECT par.ID_PRODUIT_PARTENAIRE,  ep.ID_PRODUIT,ep.NOM,ep.ID_CATEGORIE_PRODUIT,ep.ID_PRODUIT_SOUS_CATEGORIE,ep.IMAGE_1,ep.IMAGE_2,ep.IMAGE_3,ep.ID_PARTENAIRE_SERVICE,ps.NOM_ORGANISATION,u.NOM AS NOM_USER ,u.PRENOM,"
        sqlQuery += " sp.PRIX,eps.QUANTITE_TOTAL,eps.QUANTITE_VENDUS,eps.QUANTITE_RESTANTE,ps.ID_TYPE_PARTENAIRE"
        sqlQuery += " FROM  ecommerce_produits ep LEFT JOIN ecommerce_produit_stock eps ON eps.ID_PRODUIT=ep.ID_PRODUIT"
        sqlQuery += " LEFT JOIN ecommerce_produit_details epd ON epd.ID_PRODUIT_STOCK =eps.ID_PRODUIT_STOCK LEFT JOIN"
        sqlQuery += " partenaire_service ps ON ps.ID_PARTENAIRE_SERVICE= ep.ID_PARTENAIRE_SERVICE "
        sqlQuery += " LEFT JOIN ecommerce_produit_partenaire par ON par.ID_PRODUIT=ep.ID_PRODUIT "
        sqlQuery += " LEFT JOIN  ecommerce_stock_prix sp ON sp.ID_PRODUIT_STOCK=eps.ID_PRODUIT_STOCK "
        sqlQuery += " LEFT JOIN ecommerce_statut_prix st ON st.ID_STATUT=sp.ID_STATUT"
