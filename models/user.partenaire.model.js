...
const findpartenaire = async (lat, long, shop, limit = 10, offset = 0) => {
    try {
        var binds = []
        var sqlQuery = "SELECT ps.*,u.IMAGE,ps.LOGO,ps.BACKGROUND_IMAGE, ps.	EMAIL"
        if (lat && long) {
            sqlQuery += `, ( 6371 * acos( cos( radians(${lat}) ) * cos( radians( ps.LATITUDE ) ) * cos( radians(ps.LONGITUDE) - radians(${long})) + sin(radians(${lat})) * sin( radians(ps.LATITUDE)))) AS DISTANCE  `
        }
        sqlQuery += "   FROM partenaire_service ps "
...
const changeService =(NOM_ORGANISATION, TELEPHONE, NIF, EMAIL, ADRESSE_COMPLETE, LATITUDE, LONGITUDE, LOGO, BACKGROUND_IMAGE, ID_PARTENAIRE_SERVICE) => {
    try {
        var sqlQuery = "UPDATE partenaire_service SET NOM_ORGANISATION=?,TELEPHONE=?,NIF=?,EMAIL=?,ADRESSE_COMPLETE=?,LATITUDE=?,LONGITUDE=?,LOGO=?,BACKGROUND_IMAGE=? WHERE ID_PARTENAIRE_SERVICE=?";

        return query(sqlQuery, [
            NOM_ORGANISATION, TELEPHONE, NIF, EMAIL, ADRESSE_COMPLETE, LATITUDE, LONGITUDE, LOGO, BACKGROUND_IMAGE, ID_PARTENAIRE_SERVICE])
    }
    catch (error) {
