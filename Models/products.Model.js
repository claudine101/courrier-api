...
const findproducts=async()=>{
    try{
        ...
        return query("SELECT pp.NOM AS NOM_PRODUIT,pp.DESCRIPTION,pp.PRIX,pp.IMAGE_1,pp.IMAGE_2,p.NOM_ORGANISATION ,pc.NOM AS NOM_CATEGORIE,psc.NOM AS NOM_SOUS_CATEGORIE,pt.NOM NOM_TAILLE FROM ecommerce_produit_partenaire pp LEFT JOIN partenaires p ON pp.ID_PARTENAIRE=p.ID_PARTENAIRE LEFT JOIN ecommerce_produit_categorie pc ON pc.ID_CATEGORIE_PRODUIT=pp.ID_CATEGORIE_PRODUIT LEFT JOIN ecommerce_produit_sous_categorie psc ON psc.ID_PRODUIT_SOUS_CATEGORIE=pp.ID_PRODUIT_SOUS_CATEGORIE LEFT JOIN ecommerce_produit_tailles pt ON pt.ID_TAILLE=pp.ID_TAILLE WHERE 1");
    }
    catch(error)
    {
        throw error
...
const findSousCategories=async()=>{
    try{
        ...
        return query("SELECT pc.NOM AS NOM_CATEGORIE,psc.NOM AS NOM_SOUS_CATEGORIE FROM ecommerce_produit_sous_categorie psc LEFT JOIN ecommerce_produit_categorie pc ON pc.ID_CATEGORIE_PRODUIT=psc.ID_CATEGORIE_PRODUIT  WHERE 1");
    }
    catch(error)
    {
        throw error
